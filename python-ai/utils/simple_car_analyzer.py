...
class SimpleCarAnalyzer:
    """
    Простой анализатор автомобилей
    """
    ...
    def analyze_car_condition(self, image: np.ndarray) -> Dict[str, Any]:
        """
        Простой анализ состояния автомобиля
        """
        try:
    ...
    def _get_rust_description(self, percentage: float, areas_count: int) -> str:
        if percentage > 5:
            return f"Критическая ржавчина! {percentage:.1f}% поверхности поражено"
        elif percentage > 2:
            return f"Серьезная ржавчина! {percentage:.1f}% поверхности поражено"
        elif percentage > 0.5:
            return f"Заметная ржавчина! {percentage:.1f}% поверхности поражено"
        else:
    ...
    def _get_scratch_description(self, percentage: float, areas_count: int) -> str:
        if percentage > 2:
            return f"Множественные царапины! {percentage:.1f}% поверхности"
        elif percentage > 0.5:
            return f"Заметные царапины! {percentage:.1f}% поверхности"
        else:
    ...
    def _get_dent_description(self, percentage: float, areas_count: int) -> str:
        if percentage > 3:
            return f"Множественные вмятины! {percentage:.1f}% поверхности"
        elif percentage > 1:
            return f"Заметные вмятины! {percentage:.1f}% поверхности"
        else:
    ...
    def _get_crack_description(self, percentage: float, areas_count: int) -> str:
        if percentage > 1:
            return f"Критические трещины! {percentage:.1f}% поверхности"
        elif percentage > 0.3:
            return f"Серьезные трещины! {percentage:.1f}% поверхности"
        else:
